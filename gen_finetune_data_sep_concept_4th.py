...
def gen_ftdata(args, subject, dev_expls_df, val_expls_df, test_expls_df, all_data_f=None):
    ...
    for df_idx, df in enumerate([dev_expls_df, val_expls_df, test_expls_df]):
        ...
        for i in (range(df.shape[0])):
            if pd.isna(df.loc[i, "concepts"]) or pd.isna(df.loc[i, "fourth_explanations"]):
                continue
            ...
            fourth_explanations = df.loc[i, "fourth_explanations"].split(";|; ")
            for concept, concept_with_taxo_path, fourth_expl in zip(concepts, concepts_with_taxo_path, fourth_explanations):
                if "Not found" in concept_with_taxo_path:
                    concept_with_taxo_path = concept + ": Not found"
                if "Not available" in fourth_expl:
                    continue
                ...
                dialog = {
                    "messages": [
                        ...
                        {"role": "assistant", "content": fourth_expl}
                    ]
                }
                dialogs.append(dialog)
        
        os.makedirs(os.path.join(args.save_dir, "ftdata_{}_sep_taxo_path_{}_4th".format(args.expl_model_name, args.taxo_path_src), df_names[df_idx]), exist_ok=True)
        data_file = os.path.join(
            args.save_dir, "ftdata_{}_sep_taxo_path_{}_4th".format(args.expl_model_name, args.taxo_path_src), df_names[df_idx], subject + "_ftdata.jsonl"
        )
...
def main(args):
    ...
    if not os.path.exists(os.path.join(args.save_dir, "ftdata_{}_sep_taxo_path_{}_4th".format(args.expl_model_name, args.taxo_path_src))):
        os.makedirs(os.path.join(args.save_dir, "ftdata_{}_sep_taxo_path_{}_4th".format(args.expl_model_name, args.taxo_path_src)))
    all_data_f = open(
        os.path.join(
            args.save_dir, 
            "ftdata_{}_sep_taxo_path_{}_4th".format(args.expl_model_name, args.taxo_path_src), 
            "ftdata_{}_sep_taxo_path_{}_4th.jsonl".format(args.expl_model_name, args.taxo_path_src)
        ), 
        "w", encoding="utf-8"
    )

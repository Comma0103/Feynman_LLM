...
def gen_expl(args, subject, teacher_client, dev_concepts_df, val_concepts_df, test_concepts_df):
    ...
    for df_idx, df in enumerate([dev_concepts_df, val_concepts_df, test_concepts_df]):
        save_path = os.path.join(args.expl_dir, "expls_{}_from_old".format(args.expl_model_name), df_names[df_idx], subject + "_expls.csv")
        ...
        os.makedirs(os.path.join(args.expl_dir, "expls_{}_from_old".format(args.expl_model_name), df_names[df_idx]), exist_ok=True)
...
def main(args):
    ...
    if not os.path.exists(os.path.join(args.expl_dir, "expls_{}_from_old".format(args.expl_model_name))):
        os.makedirs(os.path.join(args.expl_dir, "expls_{}_from_old".format(args.expl_model_name)))
    ...
    for subject in subjects:
        ...
        dev_concepts_df = pd.read_csv(
            os.path.join(
                args.concept_dir,
                "concepts_{}_from_old".format(args.concept_model_name),
                "dev",

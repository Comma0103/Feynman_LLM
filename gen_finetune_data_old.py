...
def gen_ftdata(args, subject, dev_concepts_df, val_concepts_df, test_concepts_df, all_data_f=None):
    ...
    for df_idx, df in enumerate([dev_concepts_df, val_concepts_df, test_concepts_df]):
        ...
        os.makedirs(os.path.join(args.save_dir, "ftdata_{}".format(args.concept_model_name), df_names[df_idx]), exist_ok=True)
        data_file = os.path.join(
            args.save_dir, "ftdata_{}".format(args.concept_model_name), df_names[df_idx], subject + "_ftdata.jsonl"
        )
...
def main(args):
    ...
    if not os.path.exists(os.path.join(args.save_dir, "ftdata_{}".format(args.concept_model_name))):
        os.makedirs(os.path.join(args.save_dir, "ftdata_{}".format(args.concept_model_name)))
    all_data_f = open(os.path.join(
        args.save_dir, "ftdata_{}".format(args.concept_model_name), "ftdata_{}.jsonl".format(args.concept_model_name)), "w", encoding="utf-8")

...
def gen_2nd_expl_request(df, i):
    concepts = df.loc[i, "concepts"]
    if pd.isna(concepts):
        return None
    old_concept_expl = df.loc[i, "explanations"]
    request = (
        ...
        "{0}\n\n" + \
        "You used to give the explanation of these concepts as follow:\n" + \
        "{1}\n\n" + \
        "However, a student LM got a wrong answer after adding your explanation into its prompt when answering the question.\n\n" + \
        "Based on your understanding of these concepts, think about the reason why the student got the wrong answer, " + \
        "could you modify the explanation of these concepts to teach someone who is not familiar with them, " + \
        "or a smaller language model, so that they can answer the question right? " + \
        "Just simply list and explain the concepts you extract within 256 tokens, DO NOT use any font format or extra words:"
    ).format(concepts, old_concept_expl)
    return request
...
def eval(args, subject, model, tokenizer, dev_df, test_df, dev_expls_df, test_expls_df, teacher_client):
    cors = []
    all_probs = []
    ...
    for i in tqdm(range(test_df.shape[0])):
        ...
        new_expl = None
        if not cor:
            request = gen_2nd_expl_request(test_expls_df, i)
            expl_prompt = teacher_client.call(request) if request is not None else None
            if expl_prompt is not None:
                new_expl = expl_prompt
                expl_prompt = "Here are some concepts with explanations that may be useful:\n" + expl_prompt + "\n\n"
            else:
                expl_prompt = ""
            prompt = expl_prompt + train_prompt + prompt_end
    ...
    return cors, acc, all_probs, new_expls
...
def main(args):
    ...
    if not os.path.exists(os.path.join(args.expl_dir, "expls_{}_2nd_from_old".format(args.expl_model_name), "test")):
        os.makedirs(os.path.join(args.expl_dir, "expls_{}_2nd_from_old".format(args.expl_model_name), "test"))
    ...
    for subject in subjects:
        ...
        dev_expls_df = pd.read_csv(
            os.path.join(
                args.expl_dir,
                "expls_{}_from_old".format(args.expl_model_name),
                "dev",
                subject + "_expls.csv",
            )
        )
        ...
        cors, acc, probs, new_expls = eval(args, subject, model, tokenizer, dev_df, test_df, dev_expls_df, test_expls_df, oai_client)
        ...
        test_expls_df["new_explanations"] = new_expls
        test_expls_df.to_csv(
            os.path.join(
                args.expl_dir, "expls_{}_2nd_from_old".format(args.expl_model_name), "test", "{}_2nd_expls.csv".format(subject)
            ),
            index=None,
        )
        toc = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--expl_dir", "-c", type=str, default="explanations")
    ...
    parser.add_argument("--expl_model_name", "-cm", type=str, default="OpenAI-GPT-4o-mini")
    args = parser.parse_args()
    args.exp_name = f"{args.model_name}_feynman_{args.expl_model_name}_question_concept_2nd_from_old"
    main(args)
